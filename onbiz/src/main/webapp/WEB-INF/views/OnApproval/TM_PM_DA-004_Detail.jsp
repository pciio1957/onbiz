...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#regBtn").click(function() {
			var appdteVal = $("[name=app_appdte]").val();
			var appComVal = $("[name=app_comment]").val();
			...
			if(appdteVal == "") {
				...
				if(confirm("결재 조치를 완료하시겠습니까?\n한번 조치한 결재목록은 다시 수정하실 수 없습니다.")) {
					$("#onapproval").submit();
				}				
		...
		*/
		...
		// 승인여부 선택
		$("[name=app_state]").change(function() {
			var appCkVal = $("[name=app_state]").val();
			console.log("승인여부 : " + appCkVal);
			$("#appCk2").text(appCkVal);	
		});
		
		/*
		// 작업진행상태 선택
		$("[name=task_state]").change(function() {
			var taskCkVal = $("[name=task_state]").val();
			console.log("작업상태여부 : " + taskCkVal);
			$("#taskCk2").text(taskCkVal);	
		});		
		*/
        ...
        <div id="content-wrapper" class="d-flex flex-column">
			...
			<jsp:include page="/common/header.jsp"/>
				...
				<ol class="breadcrumb" id="pagePath">
					...
					<li class="breadcrumb-item active">결재조치</li>
				</ol>
                        ...
                        <div class="card-body">
							...
							<div class="form-group row">
				            	...
				            	<label class="col-sm-1 col-form-label text-center">작업번호</label>
				                	<div class="col-sm-5">
				                       <input name="task_no" value="${dt.task_no}" class="form-control" readonly/>
				                    </div>
				            </div>
							...
							<div class="form-group row">
				            	<label class="col-sm-1 col-form-label text-center">프로젝트</label>
				                	<div class="col-sm-5">
				                       <input value="${dt.pro_name}" class="form-control" readonly/>
				                    </div>							
							
				               	<label class="col-sm-1 col-form-label text-center">결재기간</label>
				                    <div class="col-sm-2">
				                       <input type="text" name="app_regdte" value="${dt.app_regdte}" class="form-control" readonly/>
				                    </div>
				                    <div class="col-sm-2">
				                       <input type="text" name="app_deadline" value="${dt.app_deadline}" class="form-control" readonly/>	
				                    </div>
				            </div>
							...
							<div class="form-group row">
				            	<label class="col-sm-1 col-form-label text-center">작업명</label>
				               	...
				               	<label class="col-sm-1 col-form-label text-center">승인일</label>
				                    <div class="col-sm-5">
				                       <input type="text" id="app_appdte" name="app_appdte" value="${dt.app_appdte}" placeholder="YYYY-MM-DD" class="form-control" readonly/>
				                    </div>							
				            </div>
							...
							<div class="form-group row">
				               	<label class="col-sm-1 col-form-label text-center">승인여부</label>
				                    <div class="col-sm-5">
								    	<select name="app_state" class="form-select">
								        	<option value="승인">승인</option>
									        <option value="반려">반려</option>
								    	</select>
								    	<input type="hidden" id="appCk2"/>
				                    </div>
				                    
				               	<label class="col-sm-1 col-form-label text-center">작업상태</label>
				                    <div class="col-sm-5">
				                   		<%--
				                    	<input name="task_state" value="${dt.task_state}" class="form-control" placeholder="'승인'시, '진행완료'로 입력 / '반려'시, '진행중'으로 입력"/>
				                    	 --%>
								    		
								    	<select name="task_state" class="form-select">
								        	<option >진행완료</option>
									        <option >진행중</option>
								    	</select>
								    	<input type="hidden" id="taskCk2"/>
